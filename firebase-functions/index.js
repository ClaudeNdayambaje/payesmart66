...
const envPath = path.join(__dirname, '.env.viva');
if (fs.existsSync(envPath)) {
  dotenv.config({ path: envPath });
  console.log('Fichier .env.viva chargé');
} else {
  // En production, utiliser les variables d'environnement de Firebase
  console.log('Fichier .env.viva non trouvé, utilisation des variables d\'environnement Firebase');
}

// Vérifier les fonctions.config() pour Firebase
let vivaConfig = {};
try {
  vivaConfig = functions.config().viva || {};
  console.log('Configuration Firebase chargée:', JSON.stringify(vivaConfig).replace(/client_secret":"[^"]{4}[^"]*/, 'client_secret":"****'));
} catch (e) {
  console.log('Pas de configuration Firebase disponible:', e.message);
}
...
// Middleware pour le parsing du corps de la requête et CORS
...
const clientId = process.env.VIVA_CLIENT_ID || (vivaConfig.client_id ? vivaConfig.client_id : '');
const clientSecret = process.env.VIVA_CLIENT_SECRET || (vivaConfig.client_secret ? vivaConfig.client_secret : '');

console.log('Utilisation des identifiants client suivants:');
console.log('Client ID:', clientId ? (clientId.substring(0, 4) + '...') : 'Non défini');
console.log('Client Secret:', clientSecret ? (clientSecret.substring(0, 4) + '...') : 'Non défini');
...
app.post('/viva/token', async (req, res, next) => {
  try {
...
app.post('/viva/transactions', async (req, res, next) => {
  ...
  try {
    ...
    const payload = {
      ...
      callback: {
        url: `https://us-central1-${process.env.FIREBASE_PROJECT_ID || 'votre-projet'}.cloudfunctions.net/vivaPayments/viva/callback`, // URL de callback
...
app.post('/viva/callback', (req, res) => {
...
app.get('/viva/transactions/:sessionId', (req, res) => {
...
// Exporter la fonction Firebase (version V1 pour la compatibilité)
exports.vivaPayments = functions.https.onRequest(app);
