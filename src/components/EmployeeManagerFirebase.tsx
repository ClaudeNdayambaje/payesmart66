...
const EmployeeManagerFirebase: React.FC = () => {
  ...
  const { theme } = useTheme(); // Utiliser le contexte de thème
  ...
  const handleAddEmployee = async (employeeData: Partial<Employee>, avatarFile?: File | null) => {
    try {
      ...
      // Ajouter l'employé
      const employeeId = await addEmployee(newEmployee as any);
      
      // Mettre à jour la liste des employés
      if (employeeId) {
        setEmployees(prev => [...prev, { ...newEmployee, id: employeeId } as any]);
        setShowAddEmployee(false);
      }
    } catch (err) {
      console.error("Erreur lors de l'ajout de l'employé:", err);
      setError("Impossible d'ajouter l'employé. Veuillez réessayer.");
    }
  };
  ...
  const handleUpdateEmployee = async (employeeData: Partial<Employee>, avatarFile?: File | null) => {
    ...
    try {
      ...
      if (success) {
        ...
        setTimeout(() => {
          const fetchEmployees = async () => {
            try {
              ...
              const employeesData = await getEmployees();
              console.log("Données employés rechargées après mise à jour");
              setEmployees(employeesData as any);
            } catch (err) {
        ...
        setError("La mise à jour de l'employé a échoué. Veuillez réessayer.");
      }
    } catch (err) {
      console.error("Erreur lors de la mise à jour de l'employé:", err);
      setError("Impossible de mettre à jour l'employé. Veuillez réessayer.");
    }
  };

  // Fonctions pour gérer la suppression d'un employé
  const initiateDeleteEmployee = (employeeId: string) => {
    ...
    // Vérifier si c'est l'administrateur principal (isMainAdmin)
    if (employee && employee.isMainAdmin) {
      alert('Vous ne pouvez pas supprimer l\'administrateur principal.');
      return;
    }
    
    // Vérifier si c'est un administrateur système (lié à un compte Firebase)
    if (employee && employee.firebaseUid) {
      alert('Vous ne pouvez pas supprimer un administrateur système.');
      return;
    }
    
    // Préparer les données pour la boîte de dialogue de confirmation
    const employeeFullName = `${employee.firstName} ${employee.lastName}`;
    setEmployeeToDelete({ id: employeeId, name: employeeFullName });
      ...
      <div className="flex-1 overflow-auto bg-[color:var(--color-bg)] flex-grow relative">
        {activeTab === 'list' && (
          <EmployeeList 
            ...
            onDelete={initiateDeleteEmployee}
          />
        )}
        
        {activeTab === 'shifts' && (
          <ShiftManager 
            shifts={currentEmployee?.role === 'cashier' ? shifts.filter(shift => shift.employeeId === currentEmployee.id) : shifts}
            employees={employees}
            searchQuery={searchQuery}
