...
const SupplierOrderModal: React.FC<SupplierOrderModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="bg-gray-100 dark:bg-gray-800 border-b dark:border-gray-600 sticky top-0 z-20">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
              <h3 className="font-medium text-gray-800 dark:text-white mb-3 flex items-center gap-2">
                <Package size={18} className="theme-primary-text dark:text-blue-400" />
                <span>Détails de la commande</span>
              </h3>
              
              <div className="space-y-4">
                <div>
                  <label htmlFor="deliveryDate" className="block text-sm font-medium text-gray-700 dark:text-white mb-1">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="border dark:border-gray-600 rounded-lg px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm dark:bg-gray-900 dark:text-gray-200"
                    />
                  </div>
                ...
                <div className="pt-3 border-t dark:border-gray-800">
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-gray-600 dark:text-gray-300">Produits sélectionnés:</span>
                    <span className="font-medium dark:text-white">{selectedProductsCount}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600 dark:text-gray-300">Total estimé:</span>
                    <span className="font-medium text-green-600 dark:text-green-400">{orderTotal.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="bg-white dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
              <h3 className="font-medium text-gray-800 dark:text-white mb-3 flex items-center gap-2 justify-between">
                <div className="flex items-center gap-2">
                  <ShoppingCart size={18} className="theme-primary-text dark:text-blue-400" />
                  <span>Sélection des produits</span>
                </div>
                {activeTab === 'low-stock' && lowStockProducts.length > 0 && (
                  <button 
                    onClick={selectAllLowStockProducts}
                    className="text-xs theme-primary-text hover:underline flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700"
                ...
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="border dark:border-gray-600 rounded-lg pl-10 pr-4 py-2 w-full focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm dark:bg-gray-900 dark:text-gray-200 dark:placeholder-gray-400"
                ...
                <div className="flex gap-2">
                  <div className="relative">
                    <div className="flex items-center border rounded-lg overflow-hidden">
                      <button
                        className={`px-3 py-2 text-sm font-medium ${activeTab === 'low-stock' ? 'theme-primary-bg text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
                      ...
                      <button
                        className={`px-3 py-2 text-sm font-medium ${activeTab === 'all' ? 'theme-primary-bg text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
                  ...
                  <div className="relative">
                    <div className="flex items-center border rounded-lg overflow-hidden">
                      <div className="px-2 py-2 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 flex items-center">
                        <Filter size={14} />
                      </div>
                      <select
                        value={supplierFilter}
                        onChange={(e) => setSupplierFilter(e.target.value)}
                        className="px-2 py-2 text-sm focus:outline-none border-0 dark:bg-gray-700 dark:text-gray-200"
                      ...
                      {filteredProducts.map(product => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 whitespace-nowrap">
                              <div className="text-sm text-gray-500 dark:text-gray-400">{product.supplier || 'Non spécifié'}</div>
                            </td>
                              ...
                              <div className="flex flex-col items-end">
                                <div className="flex items-center">
                                  <input
                                    ...
                                    className="w-20 border dark:border-gray-600 rounded-lg px-2 py-1.5 text-right focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-900 dark:text-gray-200"
                                    aria-label={`Quantité pour ${product.name}`}
                                  />
                                  <span className="text-sm text-gray-500 dark:text-gray-300 w-12 ml-1">unités</span>
                                </div>
        ...
        {/* État de chargement */}
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-800 bg-opacity-90 dark:bg-opacity-90 z-20">
            <div className="bg-white dark:bg-gray-900 p-4 rounded-lg border-0 dark:border-0 shadow-sm">
              <LoadingSpinner type="circle" size="lg" label="Chargement des détails de la commande..." />
            </div>
          </div>
        )}
        ...
        {successMessage && (
          <div className="absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-800 bg-opacity-90 dark:bg-opacity-90 z-10">
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 flex flex-col items-center gap-4 shadow-lg max-w-md w-full mx-4">
              <div className="bg-green-100 dark:bg-green-800/40 rounded-full p-3">
                <Check size={32} className="text-green-600 dark:text-green-400" />
              </div>
              <div className="text-center">
                <h3 className="font-medium text-green-800 dark:text-green-300 text-xl">{successMessage}</h3>
              ...
              <button 
                onClick={onClose}
                className="mt-2 px-6 py-2 bg-green-600 dark:bg-green-700 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-600 transition-colors font-medium"
        ...
        {/* Footer */}
        <div className="p-4 border-t dark:border-gray-800 theme-primary-bg-light dark:bg-gray-900 flex flex-col md:flex-row items-center justify-between gap-4 sticky bottom-0 shadow-md z-10">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button
              onClick={onClose}
              disabled={isSubmitting}
              className={`px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors font-medium flex items-center gap-1 text-sm dark:text-white ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            <div className="text-sm text-gray-700 dark:text-white">
              {selectedProductsCount > 0 ? (
                <span><b>{selectedProductsCount}</b> produit{selectedProductsCount > 1 ? 's' : ''} sélectionné{selectedProductsCount > 1 ? 's' : ''} • <b className="text-green-600 dark:text-green-400">{orderTotal.toFixed(2)} €</b></span>
              ) : (
          ...
          <button
            ...
            className={`w-full md:w-auto bg-green-600 dark:bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 dark:hover:bg-green-500 flex items-center justify-center gap-2 transition-colors text-base font-medium ${(isSubmitting || selectedProductsCount === 0) ? 'opacity-70 cursor-not-allowed' : 'shadow-md'}`}
            aria-label="Confirmer la commande"
          >
            {isSubmitting ? (
              <>
                <LoadingSpinner type="circle" size="xs" inButton={true} />
